...
- 图中每个 AutoCAD `ACAD_TABLE` → 一个 Sheet
- Sheet 名为表格标题行
- Sheet 中不包含标题行，只包含真实数据行

> 目前仅支持 **标准 TABLE 对象（ACAD_TABLE 实体）**，不支持用线段+文字“拼出来”的表格。
...
- **标题与 Sheet 名**：
  - 若第一行存在非空单元格：
    - 取该行第一个非空文本作为 **表格标题**，用作 Excel **Sheet 名**；
    - 第一行 **不写入 Excel**，只作为标题行；
  - 若第一行全为空：
    - 使用 `布局名_Table序号` 作为 Sheet 名；
    - 整个表格数据全部写入 Excel。

已知限制
--------

- 仅支持 **ACAD_TABLE**；若表格是由直线 + TEXT/MTEXT“手工绘制”，本工具不会识别。
